...
class DataCollectionWrapper(Wrapper):
    ...
    def set_successful(self, successful):
        """
        Set success flag for current episode before flushing to disk.

        Args:
            successful (bool): Whether current episode is successful.
        """
        self.successful = bool(successful)

    def flush_current_episode(self):
        """
        Flush currently buffered episode data to disk.
        """
        if self.has_interaction and (len(self.states) > 0 or len(self.action_infos) > 0):
            self._flush()
    ...
    def step(self, action):
        ...
        # Leave successful=False during practice runs so accidental task completion
        # does not get promoted into a saved demonstration.
