...
class MJGUI(Device):
    ...
    def _display_controls():
        ...
        print("s: switch active arm (for multi-arm robots)")
        print("=: switch active robot (for multi-robot envs)")
        print("")
    ...
    def input2action(self, goal_update_mode="target") -> Dict[str, np.ndarray]:
        ...
        for site_name in site_names:
            ...
            arm_names = self.all_robot_arms[self.active_robot]
            if target_name_prefix in arm_names:
                arm_index = arm_names.index(target_name_prefix)
                grasp = 1 if self.grasp_states[self.active_robot][arm_index] else -1
            else:
                # Fallback to currently active arm state.
                grasp = 1 if self.grasp else -1
